/*@webcogs_build 0.6.0 openai-gpt-5.4 2026-05-17T08:09:14.777Z
...
@webcogs_user_prompt
Write a module that displays in main, which shows an iframe that loads the URL provided by getGameUrl(). Ensure the iframe content fits in the window.
...
export class GamePreview {
  constructor(core, params) {
    ...
    this.root = this.core.mount(
      ...
      `
        <div class="preview-wrap">
          <iframe class="preview-frame" title="Game Preview"></iframe>
        </div>
      `,
      `
        :host {
    ...
    this.iframe = this.root.querySelector('.preview-frame');
    if (this.iframe) {
      this.iframe.src = this.core.getGameUrl();
    }

    this._onResize = this.updateSize.bind(this);
  ...
  updateSize() {
    if (!this.root) return;
    const host = this.root.host || this.root;
    const rect = host.getBoundingClientRect();
    const wrap = this.root.querySelector('.preview-wrap');
    if (wrap && rect.height > 0) {
