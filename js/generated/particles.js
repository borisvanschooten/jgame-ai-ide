/*@webcogs_build 0.7.0 openai-gpt-5.4 2026-06-21T21:23:05.893Z
...
@webcogs_user_prompt
Write a module that displays in main which allows to edit particle generators. The prompt should be editable in a textarea. Call setParticle on any change to the entity form to ensure it is saved. Add a build button to build a particle using buildModule(...). Add a view source button that routes to viewmodulesource with as parameters type='particles' and name=[name of the particle generator].
@webcogs_end_prompt_section*/
export class ParticleGeneratorsEditor {
  constructor(core, params) {
    ...
    this.viewSourceButton = null;
    this._suspendAutosave = false;

    const savedState = this.core.loadModuleState(this) || {};
    this.selectedName = savedState.selectedName || this.params.name || this._getFirstParticleName();
  ...
  _getFirstParticleName() {
    const names = Object.keys(this.particles);
  ...
  _mountSidebar() {
    const html = `
      <div class="side-bar-list">
        <button id="add-particle-button" type="button">Add Particle Generator</button>
        <div id="particle-list"></div>
      </div>
    `;
    this.sidebarRoot = this.core.mount('side_bar', html, '');
    const addButton = this.sidebarRoot.getElementById('add-particle-button');
    addButton.addEventListener('click', () => this._addParticle());
  }

  _mountMain() {
    const html = `
      <div class="particle-editor">
        <div id="empty-state" style="display:none;">No particle generator selected.</div>
        <div id="form-container" style="display:none;">
          <div style="display:flex; align-items:center; gap:12px; margin-bottom:12px;">
            <label for="particle-name" style="min-width:120px;">Name</label>
            <input id="particle-name" type="text" />
          </div>
          <div style="display:flex; flex-direction:column; align-items:flex-start; gap:6px; margin-bottom:12px;">
            <label for="particle-prompt">Prompt</label>
            <textarea id="particle-prompt" rows="10" style="width:100%; box-sizing:border-box;"></textarea>
          </div>
    ...
    this.nameInput = this.mainRoot.getElementById('particle-name');
    this.promptInput = this.mainRoot.getElementById('particle-prompt');
    this.buildButton = this.mainRoot.getElementById('build-button');
    this.viewSourceButton = this.mainRoot.getElementById('view-source-button');

    this.nameInput.addEventListener('input', () => this._handleNameChange());
    this.promptInput.addEventListener('input', () => this._handlePromptChange());
    this.buildButton.addEventListener('click', () => this._buildSelected());
    this.viewSourceButton.addEventListener('click', () => this._viewSourceSelected());
  }

  _renderSidebar() {
    const list = this.sidebarRoot.getElementById('particle-list');
    const names = Object.keys(this.particles).sort();
    list.innerHTML = '';

    names.forEach((name) => {
      const info = this.particles[name] || {};
  ...
  _renderMain() {
    ...
    if (!this.selectedName || !this.particles[this.selectedName]) {
    ...
    const info = this.particles[this.selectedName] || {};
    this._suspendAutosave = true;
    this.nameInput.value = this.selectedName;
    this.promptInput.value = info.prompt || '';
    this._suspendAutosave = false;
  }

  _addParticle() {
    const baseName = 'new_particle';
    let counter = 1;
    let name = baseName;
    while (this.particles[name]) {
      counter += 1;
      name = baseName + '_' + counter;
    }

    const info = { prompt: '' };
    this.core.setParticle(name, info);
    this.particles = this.core.getParticles() || {};
    this.selectedName = name;
    this._saveState();
    this._renderSidebar();
  ...
  _duplicateParticle(name) {
    const source = this.particles[name];
    if (!source) return;

    const baseName = name + '_copy';
    let counter = 1;
    let newName = baseName;
    while (this.particles[newName]) {
      counter += 1;
    ...
    this.core.setParticle(newName, copy);
    this.particles = this.core.getParticles() || {};
  ...
  _deleteParticle(name) {
    this.core.removeParticle(name);
    this.particles = this.core.getParticles() || {};

    if (this.selectedName === name) {
      this.selectedName = this._getFirstParticleName();
    }
  ...
  _handleNameChange() {
    ...
    if (!this.selectedName || !this.particles[this.selectedName]) return;

    const oldName = this.selectedName;
    const rawNewName = this.nameInput.value;
    const newName = rawNewName.trim();
    const currentInfo = this.particles[oldName] || { prompt: '' };

    if (!newName) {
      this.core.setParticle(oldName, { prompt: this.promptInput.value });
      this.particles = this.core.getParticles() || {};
      this._renderSidebar();
      return;
    }
    ...
    this.core.removeParticle(oldName);
    this.core.setParticle(newName, {
      prompt: this.promptInput.value || currentInfo.prompt || '',
    });
    this.particles = this.core.getParticles() || {};
    this.selectedName = newName;
    this._saveState();
  ...
  _handlePromptChange() {
    ...
    this.core.setParticle(this.selectedName, {
  ...
  _buildSelected() {
    if (!this.selectedName) return;
    this.core.buildModule('particles', this.selectedName);
  }
