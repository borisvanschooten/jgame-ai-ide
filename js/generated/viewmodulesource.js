/*@webcogs_build 0.6.0 openai-gpt-5.4 2026-05-22T20:31:12.332Z
...
@webcogs_user_prompt
Write a module that shows a modal dialog to view a module's source code using the prism library for syntax highlighting. Parameters are 'type' and 'name', which indicate what to pass to getModuleSourcePath(). Show a translucent static overlay at the top that shows the module name and path, and an OK button at the top that routes to hide_modal_dialog. Assume the prism library and CSS have already been loaded.
@webcogs_end_prompt_section*/
export class ViewModuleSource {
  ...
  render() {
      ...
      <div class="dialog">
        <div class="overlay-header">
          <div class="overlay-info">
            ...
            <div class="module-path" id="modulePath"></div>
          </div>
        ...
        <div class="content">
          <pre class="language-javascript"><code id="codeBlock" class="language-javascript">Loading...</code></pre>
        </div>
      </div>
    `;
    ...
    this.codeEl = this.root.getElementById("codeBlock");
    this.infoNameEl = this.root.getElementById("moduleName");
    this.infoPathEl = this.root.getElementById("modulePath");

    this.infoNameEl.textContent = this.name ? this.name : "Unnamed module";
    this.infoPathEl.textContent = "Resolving source path...";

    const okButton = this.root.getElementById("okButton");
  ...
  async loadSource() {
    try {
      const path = await this.core.getModuleSourcePath(this.type, this.name);
      if (!this.mounted) return;

      this.infoNameEl.textContent = this.name ? this.name : "Unnamed module";
      this.infoPathEl.textContent = path || "Source path not found";

      if (!path) {
        this.codeEl.textContent = "Unable to locate source code for this module.";
      ...
      if (!response.ok) {
        this.codeEl.textContent = "Failed to load source code (" + response.status + ").";
        if (typeof Prism !== "undefined" && Prism.highlightElement) {
          Prism.highlightElement(this.codeEl);
        }
        return;
      ...
      if (!this.mounted) return;
      this.infoPathEl.textContent = "Error resolving source path";
      this.codeEl.textContent = "Error loading source code: " + (err && err.message ? err.message : String(err));
      if (typeof Prism !== "undefined" && Prism.highlightElement) {
        Prism.highlightElement(this.codeEl);
      }
    }
