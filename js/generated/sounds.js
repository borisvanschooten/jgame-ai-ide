/*@webcogs_build 0.6.0 openai-gpt-5.4 2026-05-25T09:57:17.393Z
...
@webcogs_user_prompt
Write a module that displays in main which allows to edit sounds. Provide a play sound button that calls playSound, and a button 'randomize seed', which inserts a 7-digit integer in the seed field, and also plays the sound via playSound.
@webcogs_end_prompt_section*/
export class SoundsEditor {
  constructor(core, params) {
    this.core = core;
    this.params = params || {};
    this.soundsData = this.core.getSounds() || { sounds: [] };
    if (!Array.isArray(this.soundsData.sounds)) this.soundsData.sounds = [];

    const saved = this.core.loadModuleState(this) || {};
    this.selectedIndex = typeof saved.selectedIndex === 'number' ? saved.selectedIndex : (this.soundsData.sounds.length ? 0 : -1);
  ...
  saveState() {
    this.core.saveModuleState(this, { selectedIndex: this.selectedIndex });
  }
  ...
  renderSidebar() {
    ...
    const addButton = this.sidebarRoot.getElementById('add-sound');
  ...
  refreshSidebarList() {
    ...
    const list = this.sidebarRoot.getElementById('sound-list');
    if (!list) return;

    list.innerHTML = '';

    this.soundsData.sounds.forEach((sound, index) => {
      const item = document.createElement('div');
      item.className = 'item' + (index === this.selectedIndex ? ' selected' : '');

      const name = document.createElement('div');
      name.className = 'item-name';
      name.textContent = sound.name || '(unnamed)';

      const summary = document.createElement('div');
      summary.className = 'item-summary';
      summary.textContent = (sound.type || '') + (sound.seed !== undefined && sound.seed !== null && sound.seed !== '' ? ' • ' + sound.seed : '');
  ...
  addSound() {
    const newSound = {
      name: 'sound' + (this.soundsData.sounds.length + 1),
      type: 'Random',
      seed: this.generateSeed()
    };
    this.soundsData.sounds.push(newSound);
    this.selectedIndex = this.soundsData.sounds.length - 1;
    this.saveState();
    this.refreshSidebarList();
    this.renderMain();
  }

  duplicateSound(index) {
    const source = this.soundsData.sounds[index];
    if (!source) return;
    const copy = {
      name: (source.name || 'sound') + '_copy',
      type: source.type,
      seed: source.seed
    };
    this.soundsData.sounds.splice(index + 1, 0, copy);
  ...
  deleteSound(index) {
    if (index < 0 || index >= this.soundsData.sounds.length) return;
    this.soundsData.sounds.splice(index, 1);
    if (!this.soundsData.sounds.length) {
      this.selectedIndex = -1;
    } else if (this.selectedIndex >= this.soundsData.sounds.length) {
      this.selectedIndex = this.soundsData.sounds.length - 1;
    }
    this.saveState();
    this.refreshSidebarList();
  ...
  renderMain() {
    const selected = this.soundsData.sounds[this.selectedIndex] || null;
    const html = `
      <div class="sounds-editor">
        <h2>Sounds</h2>
    ...
    const editorArea = this.mainRoot.getElementById('editor-area');

    if (!selected) {
      editorArea.innerHTML = '<div class="empty-state">No sound selected.</div>';
      return;
    }
    ...
    editorArea.innerHTML = `
      <div class="form-row">
        ...
        <input type="text" id="sound-name" value="${this.escapeAttr(selected.name || '')}">
      </div>
      <div class="form-row">
        <label for="sound-type">Type</label>
        <select id="sound-type">
          ${this.renderTypeOptions(selected.type)}
        </select>
      </div>
      <div class="form-row">
        <label for="sound-seed">Seed</label>
        <input type="number" id="sound-seed" value="${this.escapeAttr(selected.seed !== undefined && selected.seed !== null ? String(selected.seed) : '')}" step="1">
      </div>
      <div class="button-row">
        <button type="button" id="play-sound">Play Sound</button>
        <button type="button" id="randomize-seed">Randomize Seed</button>
      </div>
    `;

    const nameInput = this.mainRoot.getElementById('sound-name');
    const typeSelect = this.mainRoot.getElementById('sound-type');
    const seedInput = this.mainRoot.getElementById('sound-seed');
    const playButton = this.mainRoot.getElementById('play-sound');
    const randomizeButton = this.mainRoot.getElementById('randomize-seed');
    ...
    seedInput.addEventListener('input', () => {
      ...
      selected.seed = value === '' ? '' : parseInt(value, 10);
      this.refreshSidebarList();
    });

    playButton.addEventListener('click', () => {
      if (selected.name) this.core.playSound(selected.name);
    });
  ...
  renderTypeOptions(selectedType) {
    const types = ['Random', 'Pickup', 'Powerup', 'Jump', 'Shoot', 'Blip', 'Hit', 'Explo'];
    return types.map((type) => {
      const selected = type === selectedType ? ' selected' : '';
      return '<option value="' + this.escapeAttr(type) + '"' + selected + '>' + this.escapeHtml(type) + '</option>';
    }).join('');
  }
