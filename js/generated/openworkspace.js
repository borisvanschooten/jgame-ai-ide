/*@webcogs_build 0.6.0 openai-gpt-5.4 2026-05-15T19:54:16.952Z
...
@webcogs_user_prompt
Write a module that displays in main titled 'Open workspace', that enables the user to select a directory on the local file system using pickDirectory(). Once selected, it routes to workspace_opened with as parameter 'directory' the full path string.
...
export class OpenWorkspace {
  ...
  render() {
    const html = `
      <div class="open-workspace">
        ...
        <div>
          <button id="pick-directory-btn" type="button">Select directory</button>
        </div>
    ...
    this.pickButton = this.root.getElementById('pick-directory-btn');
    this.statusEl = this.root.getElementById('status');

    this.onPickClick = this.handlePickClick.bind(this);
    this.pickButton.addEventListener('click', this.onPickClick);
  }
  ...
  async handlePickClick() {
    ...
    this.statusEl.textContent = 'Opening directory picker...';

    try {
      const directory = await this.core.pickDirectory();
      if (directory) {
        this.core.route('workspace_opened', { directory: directory });
        return;
      }
      this.statusEl.textContent = 'No directory selected.';
    } catch (e) {
      this.statusEl.textContent = 'Unable to open directory.';
    } finally {
      this.isPicking = false;
      if (this.pickButton) {
